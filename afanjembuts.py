...
async def main():
    ...
    arg.add_argument(
        "--data", "-D", default="datane.txt", help="File containing account data"
    )
    arg.add_argument(
        "--proxy",
        "-P",
        ...
        if opt == "1":
            ...
            while True:
                ...
                for no, data in enumerate(datas):
                    ...
                    if not majtod.valid:
                        continue
                    majors.append(majtod)
                tasks = [
                    asyncio.create_task(mad.start(sem)) for i, mad in enumerate(majors)
                ]
